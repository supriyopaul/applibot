...
def get_job_summary(job_description, resume, additional_info):
    ...
    job_summary_text = llm.predict(job_summary_prompt)
    return job_summary_prompt, job_summary_text

def get_cover_letter(job_description, resume, additional_info):
    ...
    cover_letter_text = llm.predict(cover_letter_prompt)
...
def main():
    ...
    try:
        ...
        if args.email:
            prompt, output = get_eoi_email(context,resume, additional_info)
        elif args.dm:
            prompt, output = get_eoi_dm(context, resume, additional_info)
        elif args.reply:
            prompt, output = get_eoi_dm_reply(context, resume, additional_info)
        elif args.summary:
            prompt, output = get_job_summary(context, resume, additional_info)
        elif args.cover_letter:
            prompt, output = get_cover_letter(context, resume, additional_info)
